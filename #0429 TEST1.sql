...
INSERT INTO DEPARTMENT VALUES('ITB','IT Billing',1);
INSERT INTO DEPARTMENT VALUES('ENG','ENGINEERING',2);
...
SELECT C.ID,C.NAME,C.AGE,D.ID FROM TEST AS C, DEPARTMENT AS D WHERE C.ID=D.ID;
SELECT C.ID AS TEST_ID, C.NAME AS TEST_NAME, C.AGE, D.DEPT 
FROM TEST AS C,DEPARTMENT AS D WHERE C.ID=D.ID;